...
import com.hit.algorithms.ExactStringMatcher;
...
public class Controller {
    ...
    public void getUserList() throws IOException, ClassNotFoundException {
        Socket socket = new Socket("localhost", 1234);
        writer = new PrintWriter(socket.getOutputStream(), true);
        writer.println("REGISTEREDLIST");
        ObjectInputStream inputStream = new ObjectInputStream(socket.getInputStream());
        users = (ArrayList<User>) inputStream.readObject();
    }

    public void RegisterUserToDB(User newUser) throws IOException {
        Socket socket = new Socket("localhost", 1234);
        String s = newUser.toString();
        writer = new PrintWriter(socket.getOutputStream(), true);
        writer.println("Register-USER#3$4:" + s);
    }
        ...
        if (!regName.getText().equalsIgnoreCase("")
            ...
            if(checkUser(regName.getText())) {
                if(checkEmail(regEmail.getText())) {
                    ...
                    newUser.username = regName.getText();
    ...
    private boolean checkUser(String username) throws IOException, ClassNotFoundException {
        //getUserList();
        for(User user : users) {
    ...
    public void login() throws IOException, ClassNotFoundException {
        ...
        boolean login = false;
        ExactStringMatcher<String> matcher = new ExactStringMatcher();
        for (User x : users) {
            if (x.username.equalsIgnoreCase(username) && matcher.match(x.password, password)) {
                login = true;
                loggedInUser.add(x);
